...
#if python version isn't 3 or over run using 2 syntax
	...
	for file_size in filesizes:
		...
		# if striping was requested we have to do a more complex calc			
					...
					#check if DU_count is integer (if not we have a partial DU)
							...
							if (no_stripes==1) and (remainder_size>1): # we have just over 1 stripe (one a bit at least!)
...
#if version of Python is over 3, run with new syntax
else:
